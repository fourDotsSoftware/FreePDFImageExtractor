...
namespace PdfImageExtractor
{
    class ColorDepthHelper
    {
        ...
        public static Bitmap ChangeColorDepth(Bitmap img, string filepath, long depth)
        {
            if (!dib.IsNull)
            ...
            {                
                ...
                dib = FreeImage.LoadFromStream(ms);


                // Check success
                if (dib.IsNull)
                {
                    Module.ShowError("Could not load image " + filepath);
                    FreeImage.Unload(dib);
                ...
                {
                    case 24:
                        dib = FreeImage.ConvertColorDepth(dib, FREE_IMAGE_COLOR_DEPTH.FICD_24_BPP, true);
                        break;
                    case 16:
                        dib = FreeImage.ConvertColorDepth(dib, FREE_IMAGE_COLOR_DEPTH.FICD_16_BPP_555, true);
                        break;
                    case 8:
                    ...
                    case 4:
                        ...
                        break;
                    case 1:
                        dib = FreeImage.ConvertColorDepth(dib, FREE_IMAGE_COLOR_DEPTH.FICD_01_BPP, true);
                        break;
                    case 32:
                        dib = FreeImage.ConvertColorDepth(dib, FREE_IMAGE_COLOR_DEPTH.FICD_32_BPP, true);
                        break;
                }

                if (FreeImage.GetBPP(dib) == (uint)depth)
                {
                ...
                throw new Exception("Error could not change color depth !");
            
        }

        public static Bitmap Change3ColorDepth(Bitmap img, string filepath, long depth)
        {
        ...
        public static Bitmap C1hangeColorDepth(Bitmap img,string filepath,int maxcolors,int maxbits)
        {        
            /*
            PixelFormat pixelfmt=PixelFormat.Format32bppArgb;
            switch (depth)
            {
                case 32:
                    pixelfmt = PixelFormat.Format32bppArgb;
                    break;
                case 24:
                    pixelfmt=PixelFormat.Format24bppRgb;
                    break;
                case 16:
                    pixelfmt=PixelFormat.Format16bppArgb1555;
                    break;
                case 8:
                    pixelfmt=PixelFormat.Format8bppIndexed;
                    break;
                case 4:
                    pixelfmt=PixelFormat.Format4bppIndexed;
                    break;
                case 1:
                    pixelfmt=PixelFormat.Format1bppIndexed;
                    break;
            }
            */          
                        
            /*
            Bitmap imgnew=new Bitmap(img.Width,img.Height,pixelfmt);
            Bitmap bmp = new Bitmap(imgnew);
            Graphics g = Graphics.FromImage(bmp);
            g.DrawImage(img, 0, 0);
            g.Dispose();
            */

            OctreeQuantizer objQuantizer = new OctreeQuantizer(maxcolors,maxbits);
            Bitmap bmp = objQuantizer.Quantize(img);
          ...
          /*
            ...
            GC.Collect();
            return bmp.Clone(new Rectangle(0, 0, img.Width, img.Height), pixelfmt);
          */
