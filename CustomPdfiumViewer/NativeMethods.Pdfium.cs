...
namespace PdfiumViewer
{
    partial class NativeMethods
    {
        ...
        */
        ...
        public static int FPDFPageObj_GetType(IntPtr page_object)
        {
            lock (LockString)
            {
                return Imports.FPDFPageObj_GetType(page_object);
            }
        }        

        public static IntPtr FPDFImageObj_GetRenderedBitmap(IntPtr document,
                               IntPtr page,
                               IntPtr image_object)
        {
            lock (LockString)
            {
                return Imports.FPDFImageObj_GetRenderedBitmap(document, page, image_object);
            }
        }

        public static IntPtr FPDF_LoadDocument(string filePath,string password = "")
        {            
            lock (LockString)
            {
                //3return Imports.FPDF_LoadDocument(filePath,password);
                try
                {                    
                    ...
                    int _id = StreamManager.Register(stream);

                    var document = NativeMethods.FPDF_LoadCustomDocument(stream, password, _id);
                    if (document == IntPtr.Zero)
                    {
                        uint err = Imports.FPDF_GetLastError();

                        //MessageBox.Show("Last error=" + err.ToString());
                    }


                    //return Imports.FPDF_LoadDocument(new StringBuilder(filePath), null);
        ...
        public static FPDF_IMAGEOBJ_METADATA FPDFImageObj_GetImageMetadata(IntPtr image_object,
                              IntPtr page,
                              IntPtr metadata)
        {
            lock (LockString)
            {
                bool suc=Imports.FPDFImageObj_GetImageMetadata(image_object,
                              page,
                              metadata);

                if (!suc)
                {                    
                    return new FPDF_IMAGEOBJ_METADATA();
                }

                FPDF_IMAGEOBJ_METADATA newmetadata = (FPDF_IMAGEOBJ_METADATA)Marshal.PtrToStructure(metadata, typeof(FPDF_IMAGEOBJ_METADATA));
        ...
        public static ulong FPDFImageObj_GetImageDataDecoded(IntPtr image_object,
                                 byte[] buffer,
                                 ulong buflen)        
        {
            lock (LockString)
            {
                return Imports.FPDFImageObj_GetImageDataDecoded(image_object,
                                 buffer,
                                 buflen);
            }
        }

        public static int FPDFBitmap_GetFormat(IntPtr bitmap)                                 
        {
            lock (LockString)
            {
                return Imports.FPDFBitmap_GetFormat(bitmap);
            }
        }
        ...
        public static IntPtr FPDFImageObj_GetBitmap(IntPtr page_object)
        {
            lock (LockString)
            {
                return Imports.FPDFImageObj_GetBitmap(page_object);
            }
        }
        ...
        public static int FPDFBitmap_GetWidth(IntPtr bitmap)
        {
            lock (LockString)
            {
                return Imports.FPDFBitmap_GetWidth(bitmap);
            }
        }
        ...
        public static int FPDFBitmap_GetHeight(IntPtr bitmap)
        {
            lock (LockString)
            {
                return Imports.FPDFBitmap_GetHeight(bitmap);
            }
        }
        ...
        public static uint FPDFImageObj_GetImageDataRaw(IntPtr page_object,byte[] buffer,uint image_length)
        {
            lock (LockString)
            {
                return Imports.FPDFImageObj_GetImageDataRaw(page_object, buffer, image_length);
            }
        }       
        ...
        public static class Imports
        {
            ...
            IntPtr FPDFImageObj_GetRenderedBitmap(IntPtr document,
                               IntPtr page,
                               IntPtr image_object);

            [DllImport("pdfium.dll")]
            public static extern
            IntPtr FPDF_LoadDocument(StringBuilder filePath, StringBuilder password);

            [DllImport("pdfium.dll")]
            public static extern
            bool FPDFImageObj_GetImageMetadata(IntPtr image_object,
                              IntPtr page,
                              IntPtr metadata);
            ...
            [DllImport("pdfium.dll")]
            public static extern
            ulong FPDFImageObj_GetImageDataDecoded(IntPtr image_object,
                                 byte[] buffer,
                                 ulong buflen);

            [DllImport("pdfium.dll")]
            public static extern
            int FPDFBitmap_GetFormat(IntPtr bitmap);

            [DllImport("pdfium.dll")]
            public static extern
            IntPtr FPDFBitmap_GetBuffer(IntPtr bitmap);

            [DllImport("pdfium.dll")]
            public static extern
            IntPtr FPDFImageObj_GetBitmap(IntPtr page_object);


            [DllImport("pdfium.dll")]
            public static extern
            int FPDFBitmap_GetWidth(IntPtr bitmap);

            [DllImport("pdfium.dll")]
            public static extern
            int FPDFBitmap_GetHeight(IntPtr bitmap);
            ...
            [DllImport("pdfium.dll")]
            public static extern
            uint FPDFImageObj_GetImageDataRaw(IntPtr page_object, byte[] buffer, uint image_length);
            /////////////////////////////////////////end
